...
-- Products and Brands
# SELECT p.name AS product_name, b.name AS brand_name, pb.product_id, pb.brand_id, p.price * (1 + b.brand_markup) AS price
# FROM product p
# JOIN product_brand pb ON p.id = pb.product_id
# JOIN brand b ON b.id = pb.brand_id;

-- Orders
# SELECT brand, product, qty, price_per_unit, ROUND(qty * price_per_unit, 2) AS total_price
# FROM order_product_brand;

-- Average price per product
# SELECT p.name AS product_name, ROUND(AVG(p.price * (1 + b.brand_markup)),2) AS avg_price
# FROM product p
# JOIN product_brand pb ON p.id = pb.product_id
# JOIN brand b ON b.id = pb.brand_id
# GROUP BY 1;
...
-- Top brands sold
# SELECT brand, COUNT(*) AS total_products_sold
# FROM order_product_brand
# GROUP BY 1
...
-- Top products sold
# SELECT product, COUNT(*) AS total_products_sold
# FROM order_product_brand
# GROUP BY 1
# ORDER BY total_products_sold DESC;

-- Top products sold by brand
# SELECT brand, product, COUNT(*) AS total_products_sold
# FROM order_product_brand
# GROUP BY 1, 2
# ORDER BY total_products_sold DESC;

-- Top products sold by brand with total price
# SELECT brand, product, COUNT(*) AS total_products_sold, ROUND(SUM(qty * price_per_unit), 2) AS total_price
# FROM order_product_brand
# GROUP BY 1, 2
# ORDER BY total_products_sold DESC;

-- Top brands sold with revenue
# SELECT brand, COUNT(*) AS total_products_sold, ROUND(SUM(qty * price_per_unit), 2) AS total_revenue
# FROM order_product_brand
# GROUP BY 1
# ORDER BY total_products_sold DESC;
